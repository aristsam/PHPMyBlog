<?php include "header.php";
      ...
      $sql="SELECT * FROM blog LEFT JOIN categories 
      ON blog.category=categories.cat_id  LEFT JOIN user
      ON blog.author_id=user.user_id ORDER BY blog.publish_date DESC limit $offset,$limit";
...
<div class="container mt-2">
	<div class="row">
		<div class="col-lg-8">
				...
				<div class="card-body d-flex blog_flex">
					...
					<div class="flex-grow-1 flex-part2">
						...
						<ul>
							...
							<li class="me-2">
								<a href=""> <span><i class="fa fa-calendar-o" aria-hidden="true">
                                </i></span> <?= $result['publish_date']  ?> </a>
							</li>
							<li>
								<a href="category.php?id=<?= $result['cat_id'] ?>" class="text-primary">
								 <span><i class="fa fa-tag" aria-hidden="true">                                    
                                </i></span><?= $result['cat_name'] ?></a>
						    </li>
						</ul>
					</div>
				</div>
			...
			$run2=mysqli_query($config,$pagination);
			$total_post=mysqli_num_rows($run2);
			...
			<ul class="pagination pt-2 pb-5">
				...
				<li class="page-item <?= ($i==$page) ? $active="active" : "" ; ?>"><a href="index.php?page=<?= $i ?>" class="page-link">
					<?= $i ?>
				</a></li>
				<?php } ?>
			</ul>
